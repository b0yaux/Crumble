...
local amix = addNode("AudioMixer", "audioMix")
connect(vmix, addNode("ScreenOutput", "viewport"))
connect(amix, addNode("SpeakersOutput", "audioOut"))
...
function update()
    ...
    --local baseOp = math.exp((phase - 1) * 1.8) * maxOpacity  -- black → white
    local baseOp = (1 - phase) ^ 0.5 * maxOpacity  -- white → black (gentle curve)
