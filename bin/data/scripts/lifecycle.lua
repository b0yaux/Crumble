-- lifecycle.lua
-- Dynamic node creation/destruction from update().
-- Tests birth, destroy, and population management under rapid cycling.
-- Useful for validating the node leak fix (addNode idempotency + destroy cleanup).
...
function update()
    ...
    if math.floor(bar) ~= math.floor(bar - (1/60) * (120/60)) then
        print("[LIFECYCLE] bar=" .. string.format("%.2f", bar)
            .. " pop=" .. #nodes
            .. " born=" .. total_born
            .. " died=" .. total_died)
    end
